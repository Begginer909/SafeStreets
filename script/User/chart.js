...
const updateChart = (data, viewMode, startDate, endDate) => {
      ...
      const capitalize = (str) => {
        ...
        const chartWidth = Math.max(1500, labels.length * 80); // 80px per label
...
// Set predefined date range on DOMContentLoaded
document.addEventListener("DOMContentLoaded", async () => {
  ...
  dateRangeInput.value = `${formatDate(yesterday)} to ${formatDate(today)}`;

  // Load the chart with the predefined date range
  const viewMode = document.getElementById('viewMode').value; // Get the current view mode
  const data = await fetchChartData(viewMode, formatDate(yesterday), formatDate(today));
  updateChart(data, viewMode, formatDate(yesterday), formatDate(today));
});
